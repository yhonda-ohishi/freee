...
- **起動ディレクトリ**: 作業は `c:reee` を cwd にして起動する。別ディレクトリだと `.claude/settings.json` が未ロードで hooks が黙って無効になる（詳細は `repo-cwd-guard` skill）。
- **引き継ぎ**: セッション開始時に `handoff/` の最新ファイルを読む（実データを含むためローカル専用）。
...
- `repo-cwd-guard` — 起動ディレクトリの確認（cwd 外だと hooks が無効になる）
