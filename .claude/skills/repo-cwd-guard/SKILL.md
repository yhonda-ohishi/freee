---
name: repo-cwd-guard
description: freee 会計のデータ (仕訳・証憑・バックアップ) を扱う前に、セッションが c:\freee で起動しているかを確認するためのスキル。「別ディレクトリから freee を触ろうとしている」「MCP ツールが見当たらない」「hook が鳴っていない」「監査ログに残っていない」「同期されていない」といった状況、および repo-cwd-guard の警告が出たときに参照する。書き込み (仕訳の登録・更新、取引先の作成) を行う前には必ず確認すること。
---
...
**freee 会計のデータに書き込む作業は、必ず `c:\freee` を cwd にして起動したセッションで行う。**
...
| 失われるもの | 影響 |
|---|---|
| `.mcp.json` の freee MCP サーバー | MCP ツールが存在しなくなる。HTTP 直叩き等の迂回に流れやすい |
...
- 銀行同期済み口座の仕訳は、そもそも API で取引を作らない。freee Web 画面の「自動で経理」から
  登録するようユーザーに案内する (CLAUDE.md の hard constraint)。
